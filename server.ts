...
async function startServer() {
  ...
  app.post('/api/auth/login', (req, res) => {
    try {
      ...
      if (!inputId || !inputPass) return res.status(400).json({ success: false, error: 'Введите логин/email и пароль' });
      ...
      return res.status(401).json({ success: false, error: 'Неверный логин, email или пароль администратора' });
    } catch (err: any) {
  ...
  app.get('/api/auth/verify', (req, res) => {
    ...
    const token = authHeader.replace(/^Bearer\s+/i, '').trim();
    if (!isValidAdminToken(token)) return res.status(401).json({ success: false, error: 'Токен недействителен' });
    
    const admin = loadAdmin();
    res.json({ success: true, user: { username: admin.username, email: admin.email, role: 'admin' } });
  });
  ...
  app.get('/api/projects', (req, res) => {
    try {
      ...
      if (rewardType && typeof rewardType === 'string') {
        ...
        projects = projects.filter((p) => p.reward.rewardTypes?.some((t) => types.includes(t)));
      }
      ...
      if (tier && typeof tier === 'string') {
        if (tier === 'tier1') projects = projects.filter((p) => p.investors.some((i) => i.tier === 'Tier 1'));
        else if (tier === 'tier2') projects = projects.filter((p) => p.investors.some((i) => i.tier === 'Tier 2'));
      }
  ...
  app.post('/api/projects', requireAdminAuth, (req, res) => {
    try {
      const body = req.body;
      if (!body.name || !body.name.trim()) return res.status(400).json({ success: false, error: 'Название проекта обязательно' });
  ...
  app.put('/api/projects/:id', requireAdminAuth, (req, res) => {
    try {
      ...
      if (index === -1) return res.status(404).json({ success: false, error: 'Проект не найден' });
      ...
      const updatedProject: Project = {
        ...
        tokenomics: { ...existing.tokenomics, ...(body.tokenomics || {}) },
  ...
  app.delete('/api/projects/:id', requireAdminAuth, (req, res) => {
    try {
      ...
      if (filtered.length === projects.length) return res.status(404).json({ success: false, error: 'Проект не найден' });
  ...
  app.put('/api/activities/:id/toggle', (req, res) => {
    try {
      ...
      if (!targetActivity) return res.status(404).json({ success: false, error: 'Активность не найдена' });
  ...
  app.post('/api/favorites/:id', (req, res) => {
    try {
      ...
      if (req.body?.priority !== undefined) project.priority = Number(req.body.priority);
  ...
  // ВАЖНО: Подключение фронтенда (Vite) к бэкенду
